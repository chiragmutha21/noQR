...
import {
  ...
  Video, Music, Image as ImageIcon, Type, FileText, Plus, X, Paperclip, Loader2, Download, Edit2, UploadCloud, BarChart2
} from "lucide-react";
  ...
  return (
    <div className="max-w-md mx-auto relative px-1 sm:px-6 pb-20 sm:pb-32 pt-2 sm:pt-10">
      ...
      >
        ...
        <Scan className="w-3.5 h-3.5 sm:w-6 sm:h-6 text-white relative z-10 animate-pulse" />
        <span className="text-white font-black uppercase tracking-widest text-[8px] sm:text-lg relative z-10">Launch Scanner</span>
      </Link>
      ...
      {/* 6. Add New Target Bottom Button - Centered */}
      {!loading && filteredContents.length > 0 && (
        <div className="mt-4 sm:mt-12 flex justify-center pb-4 sm:pb-8">
          <Link
            href="/upload"
            className="inline-flex items-center gap-1 sm:gap-3 px-3 py-1.5 sm:px-8 sm:py-4 bg-zinc-900 border border-white/5 text-zinc-300 font-bold uppercase tracking-widest text-[6px] sm:text-[10px] rounded-full shadow-lg sm:shadow-2xl active:scale-95 transition-all"
          >
            <Plus className="w-2 h-2 sm:w-4 sm:h-4 text-blue-500" />
            Add New Target
          </Link>
        </div>
      )}
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-6">
                ...
                <button onClick={() => setShowAnalytics(null)} className="p-2 text-zinc-500"><X className="w-6 h-6" /></button>
              </div>

              <div className="space-y-6">
                <div className="bg-zinc-800/50 border border-white/5 rounded-[24px] p-6 text-center">
                  <h4 className="text-[10px] font-black uppercase text-zinc-500 tracking-widest mb-1">Total Scans</h4>
                  <p className="text-5xl font-black text-white">{showAnalytics.analytics?.totalScans || 0}</p>
                </div>
