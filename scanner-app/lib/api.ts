...
// Step 4: Analytics
export const logEngagement = (contentId: string, seconds: number) => {
    const formData = new FormData();
    formData.append('contentId', contentId);
    formData.append('seconds', seconds.toString());
    return api.post('/log-engagement', formData);
};

export const logCtaClick = (contentId: string) => {
    return api.post('/log-cta-click', { contentId });
};
