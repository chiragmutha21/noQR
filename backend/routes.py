...
async def record_engagement(content_id: str, seconds: float):
    try:
        col = get_images_collection()
        await col.update_one(
            {"contentId": content_id},
            {"$inc": {"analytics.engagementTime": seconds}}
        )
    except Exception as e:
        print(f"Failed to record engagement: {e}")

async def record_scan_analytics(content_id: str, ip: str):
    print(f"DEBUG: Recording scan analytics for {content_id} from {ip}")
    try:
        country = get_country_for_ip(ip)
        col = get_images_collection()
        # Using $inc will create the field if it doesn't exist, 
        # but for complex nested paths, MongoDB sometimes likes the root to exist.
        ...
        print(f"DEBUG: Updated {result.modified_count} docs for {content_id}")
    except Exception as e:
...
async def get_content_details(content_id: str):
    """Fetch details for a single content item, including analytics."""
    col = get_images_collection()
    doc = await col.find_one({"contentId": content_id})
    if not doc:
        raise HTTPException(status_code=404, detail="Content not found")
    
    # Ensure ID formatting matches expected response
    doc["_id"] = str(doc.get("_id", ""))
    return ARContentResponse(**doc)
...
async def scan_frame(
    ...
    print(f"--- DEBUG: SCAN REQUEST RECEIVED FROM {request.client.host if request.client else 'Unknown'} ---")
    """
    ...
    try:
        ...
        print(f"DEBUG: Triggering background analytics for {content_id}")
...
async def log_cta_click(payload: ClickRequest):
    """Log a click on a call-to-action button."""
    try:
        col = get_images_collection()
        await col.update_one(
...
async def log_engagement_time(contentId: str = Form(...), seconds: float = Form(...)):
    """Log time spent viewing a target."""
    await record_engagement(contentId, seconds)
    return {"status": "ok"}
